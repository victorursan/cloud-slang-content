########################################################################################################################
#!!
#! @description: This flow adds a server to the Zabbix monitored hosts. It contains a locking mechanism that prevents 
#!               you from register simultaneously more servers with the same name.
#!
...
#! @input agent_port: The port on which the agent listens for connections from server.
...
#!                        error message.
#! @output json_template_list: Generated description
#! @output status_code: Generated description
#! @output json_response: Generated description
#! @output zabbix_host_group_id: Generated description
#!
  ...
  workflow:
    - get_templates:
        ...
        publish: 
          ...
          - json_template_list 
        navigate: 
          - SUCCESS: get_zabbix_host_group_id_json
          - FAILURE: set_failure_message
        
    - register_server_in_zabbix:
        do:
    ...
    - get_zabbix_host_group_id:
        do:
          get_zabbix_host_group_id: 
            - object: ${ json_response } 
            - specific_error: ${'Hostgroup "' + host_group + '" not found.'} 
            - key: 'result[0].groupid' 
        publish: 
          - return_result
          - zabbix_host_group_id 
        navigate: 
          - SUCCESS: register_server_in_zabbix
          - FAILURE: set_failure_message
        
    - get_zabbix_host_group_id_json:
        do:
          http.http_client_post:
            - url: ${zabbix_protocol + '://' + zabbix_host + ':' + zabbix_port + '/zabbix/api_jsonrpc.php'} 
            - body: |
    ...
    - check_zabbix_response_1:
        ...
        publish: 
          - return_result 
        navigate: 
          - SUCCESS: get_zabbix_host_group_id
          - FAILURE: set_failure_message
    ...
    - set_success_message:
        do:
          utils.noop:
            - text: ${'Successfully added the server "' + server_name + '" to the monitored list.'}
        publish:
          - return_result: ${ text }
        navigate:
          - SUCCESS: SUCCESS
